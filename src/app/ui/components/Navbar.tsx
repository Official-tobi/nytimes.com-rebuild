...
function Navbar() {
  ...
  const getMarketData = async (stocksTicker: string) => {
    await fetch(
      ...
      .then((data) => {
        ...
        let priceData = data.results[0];
        let priceChange = priceData.c - priceData.o;
        let percentagePriceChange: number = (priceChange / priceData.o) * 100;
        let newMarketData = { ticker, percentagePriceChange };
        console.log(newMarketData);
        setStockMarketDataArr((prevMarketDataArr) => [
          ...prevMarketDataArr,
          newMarketData,
        ]);
      })
  ...
  return (
    <>
      <nav className="px-20 w-full grid grid-cols-3 text-smi items-center gap-y-2 py-3.5">
        ...
        <div className="justify-self-center">
          <ul className="flex">
            <NavButton href="/">u.s.</NavButton>
            <NavButton href="/">international</NavButton>
            <NavButton href="/">canada</NavButton>
            <NavButton href="/">Español</NavButton>
            <NavButton href="/">中文</NavButton>
          </ul>
        </div>
        <div className="justify-self-end flex gap-x-3">
          <NavAction href="/">subscribe for $0.25/week</NavAction>
          <NavAction href="/">log in</NavAction>
        </div>
        <div>
